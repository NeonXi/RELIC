...
WARFRAME-RELIC 是 Warframe（星际战甲）游戏辅助工具，提供遗物出入库查询、物品掉落查询、市场价格查询、中英翻译等功能。基于 PySide6 + RapidOCR + dxcam 技术栈，以半透明覆盖层方式在游戏画面上无侵入显示结果。
...
- 信号定义在类顶部，紧跟类文档字符串之后
- 使用 `Signal` 定义信号（来自 PySide6.QtCore），类型注解清晰
